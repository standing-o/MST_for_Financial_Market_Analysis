...
mean_ntl <- matrix(0, ncol=12, nrow=1)
for (i in 1:12){
  ...
  ntl <- matrix(0, ncol=num, nrow=1)

  
  ntl[,1] <- (1/(length(df_mst$weight)))*sum(df_mst$weight)
  
  modif_mst <- mst
  
  
  
  for (j in 1:(num-1)){  ## num-1
    for (k in 1:(n-1)){  ##  n-1
    ...
    ntl[,j+1] <- (1/(length(df_modif_mst$weight)))*sum(df_modif_mst$weight)
...
write_xlsx(as.data.frame(mean_ntl), path = "2_mean_ntl.xlsx")
