...
for (i in 1:1){
  ...
  set.seed(seed_num)
  if (i==1){
    coords<-layout_(modif_mst, with_lgl())
  }
  
  png(filename = paste("modif_mst_k0.png"), width=800, height=800, unit="px")
  plot.igraph(mst, layout = coords, vertex.label.font = 2, edge.width=3, edge.color = 'black',
              vertex.size=25, vertex.color = 'lightblue', vertex.label.cex = 2, 
  ...
  for (j in 1:(num-1)){  ## n/2-1, num-1
    ...
    png(filename = paste("modif_mst_k",j,".png", sep=''), width=800, height=800, unit="px")
    plot.igraph(modif_mst, layout = coords, vertex.label.font = 2, edge.width=3, edge.color = 'black',
                vertex.size=25, vertex.color = 'lightblue', vertex.label.cex = 2, 
                vertex.label.color = 'black')
    dev.off()
  }
